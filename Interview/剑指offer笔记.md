...
源文件到可执行文件需要经过四步：
1. 预编译(`gcc -E`)。预处理器处理如`#include`、`#define`等预编译指令，生成`.i`文件。`.i`文件其实就是将一些预编译指令进行简单文本替换后的源文件。
2. 编译(`gcc -c`)。编译器进行词法分析、语法分析、中间代码生成、目标代码生成、优化，生成`.s`文件。`.s`文件就是由汇编语言构成的文件。
3. 汇编(`gcc -S`)。汇编器把汇编码转换成机器码，生成`.o`文件。`.o`文件其实就是目标文件，里面是机器码，但是需要重定位。
4. 链接(`ld`)。链接器对输入的目标文件进行抵制和空间分配、符号决议、重定位，输出可执行文件，即`.out`文件。
...
这个问题的本质是，信道不可靠，但是通信双方需要就某个问题达成一致，而要解决这个问题，无论你要传输什么信息，三次通信是理论上的最小值。

而这个问题就是通信双方的初始序列号ISN。它是可靠传输的保证，通信双方需要知道对方的初始序列号，以便能够达到不丢失、有序的将数据交给用户。
