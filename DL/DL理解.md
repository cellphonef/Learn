...
特征就是可以清晰描述一个对象的各种属性集合（本质上就是一组数字）。
...
CNN无法很好的处理序列信息的关键是其网络没有记忆功能，无法保留历史信息（上下文），因此出现了循环神经网络RNN。
...
## 5. Seq2seq model with attention
...
ViT是将Transformer模型应用在CV领域的开山之作。

ViT直接使用Transfomer Encoder替换CNN网路，进行图像分类。
...
CNN是可以接受任何尺寸的输入的，只不过不同的输出会得到不同的输出。但FC是固定尺寸的，只能接受固定尺寸的输入。